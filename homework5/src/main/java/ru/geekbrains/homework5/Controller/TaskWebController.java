package ru.geekbrains.homework5.Controller;

import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.*;
import ru.geekbrains.homework5.Model.TaskStatus;
import ru.geekbrains.homework5.Service.NotificationService;
import ru.geekbrains.homework5.Service.TaskService;

@Controller
@RequestMapping("/")
@RequiredArgsConstructor
public class TaskWebController {
    private final TaskService service;
    private final NotificationService notificationService;


    @GetMapping
    public String homePage() {
        return "forward:index.html";
    }

    @GetMapping("/tasks")
    public String tasksPage(Model model) {
        model.addAttribute("tasks", service.getTasks());
        return "tasks";
    }

    @GetMapping(value = "/new")
    public String addTask(Model model) {
        model.addAttribute("tasks", service.getTasks());
        return "new";
    }

    @PostMapping(value = "/delete")
    public String deleteTask(@RequestParam("id") Long id, Model model) {
        notificationService.notify("Delete was active " + id);
        service.deleteTaskById(id);
        model.addAttribute("tasks", service.getTasks());
        return "redirect:/tasks";
    }

    @PostMapping(value = "/edit")
    public String updateTask(@RequestParam("id") Long id,
                             @RequestParam("description") String description,
                             @RequestParam("status") TaskStatus status,
                             Model model) {
        notificationService.notify("Update Task{" +
                "id=" + id +
                ", description='" + description + '\'' +
                ", status=" + status +
                '}');
        service.updateTaskById(description, id);
        service.setTaskStatus(status, id);
        model.addAttribute("tasks", service.getTasks());
        return "redirect:/tasks";
    }

    @PostMapping(value = "/add")
    public String createTask(@RequestParam("description") String description,
                             Model model) {
        notificationService.notify("Create Task " + service.createTask(description));
        model.addAttribute("tasks", service.getTasks());
        return "redirect:/tasks";
    }

    @PostMapping(value = "/filter")
    public String filterTask(@RequestParam("status") String status, Model model) {
        switch (status) {
            case "no_filter" -> {
                model.addAttribute("tasks", service.getTasks());
                model.addAttribute("status", status);
            }
            case "new", "proc", "comp" -> {
                model.addAttribute("tasks", service.getTasksByStatus(TaskStatus.convert(status)));
                model.addAttribute("status", status);
            }
        }
        notificationService.notify("Filter: " + status);
        return "tasks";
    }
}
